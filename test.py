...
if (prompt_number not in [1, 2]) or (language not in ["ITA", "ENG"]) or not adapter:
    ...
    print("Example: python test.py --lang ENG --prompt_number 2 --adapter QLORA --base_model 'meta-llama/Meta-Llama-3-8B-Instruct")
    sys.exit(1)
...
def generate_prompt_str(claim, evidence,adddoc): 
...
def batch_inference(prompts_with_claim, tokenizer, model):
    inputs = tokenizer(prompts_with_claim, return_tensors='pt', padding=True,truncation=True).to("cuda")
    generation_config = model.generation_config
    generation_config.pad_token_id = tokenizer.eos_token_id
    generation_config.do_sample = False
    max_length = inputs['input_ids'].size(1) + 7
    outputs = model.generate(**inputs, max_length=max_length, temperature=0.1, generation_config=generation_config)
    
    texts = [tokenizer.decode(output, skip_special_tokens=True) for output in outputs]
    results = []
    for text in texts:
        if language == "ENG":
            result = text.split("### Answer:")[-1].replace("\n", "")
        elif language == "ITA":
            result = text.split("### Risposta:")[-1].replace("\n", "")
...
#============================================		
with open("GOLD_BEST_"+language+"_pr_"+str(prompt_number)+"_ADD_"+str(adddoc)+"_SPLIT_"+str(splitevidence)+'output_pred_llm.jsonl', 'w') as output_pred_llm:
...
print("GOLD_BEST_"+language+"_pr_"+str(prompt_number)+"_ADD_"+str(adddoc)+"_SPLIT_"+str(splitevidence))
